...
class Sisi {
    ...
    public Sisi(String destination, int weight) { //konstruktor untuk membuat objek sisi
        this.destination = destination;
        this.weight = weight;
    }
}

//class utama untuk algoritma dijkstragraph
public class DijkstraGraphTraversal {
    private Map<String, List<Sisi>> adjacencyList = new HashMap<>(); //unutk menyimpan graf
    ...
    public void addEdge(String source, String destination, int weight) {
        ...
        addNode(source);  //pastikan simpul sumber dan tujuan ada di graf
        addNode(destination);

        adjacencyList.get(source).add(new Sisi(destination, weight)); //uuntuk graf berarah 
        
        adjacencyList.get(destination).add(new Sisi(source, weight)); //tambhakn sisi sebalik nya jika graf tak berarah
    }


    //metode untuk menjalankan algoritma dijkstra
    public int dijkstra(String start, String end) {
        PriorityQueue<Map.Entry<String, Integer>> pq = new PriorityQueue<>(Map.Entry.comparingByValue());
        Map<String, Integer> distances = new HashMap<>(); //map untuk menyimpan jarak dari simpul awal ke setiap simpul
        Set<String> settled = new HashSet<>(); //untk menyimpan simpul yang sudah selesai di proses


        //inisialisasi jarak semua simpul dengan nilai tak hingga
        ...
        //perulangan algoritma djikstra
        while (!pq.isEmpty()) { 
            String current = pq.poll().getKey(); //abmbil simpul dengn jrak terdekat 
            if (current.equals(end)) { //jika simpul tujuan di temukan,kembalkan jaraknya
                return distances.get(end);
            }

            if (settled.contains(current)) { // lewati simpul yang sudah selsesai di proses
                continue;
            }
        ...
        return distances.get(end); //kembalikan jarak simpul tujuan (jika ditemukan)
    }
